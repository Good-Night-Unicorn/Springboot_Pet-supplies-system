	...
	<!--  -->
		...
		<div class="detail-preview">
			...
			<div class="detail2">
				...
				<div class="desc-text" v-html="detail.neirong"></div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getDiscussList(page) {
				this.$http.get('discusstongzhigonggao/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusstongzhigonggao/update',row).then(rs=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discusstongzhigonggao/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
