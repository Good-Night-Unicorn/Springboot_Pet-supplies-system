			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.fuwuneirong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">商家号</div>
						<div class="text "  >{{detail.shangjiahao}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shouhoufuwu','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shouhoufuwu','删除')" @click="delClick">删除</el-button>
						<el-button class="auditBtn" v-if="btnAuth('shouhoufuwu','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.leixing;
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此售后服务？') .then(_ => {
